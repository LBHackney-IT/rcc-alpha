  ...
  describe '#get' do
    ...
    it 'raises an exception for an invalid json response' do
      ...
      expect { json_api.get('properties?postcode=A1 1AA') }
        .to raise_error(JsonApi::InvalidResponseError, "743: unexpected token at '<html><body>An error occurred</body></html>'")
    end
    ...
    context 'when the response was not valid JSON' do
      it 'raises an exception' do
        ...
        expect { json_api.post('/hackneyrepairs/v1/repairs', {}) }
          .to raise_error(JsonApi::InvalidResponseError, "743: unexpected token at 'not found'")
      end
    end
